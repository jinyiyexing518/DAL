...
class HRSCDataset(data.Dataset):
    ...
    def _load_annotation(self, index):
        ...
        with open(filename,'r',encoding='utf-8-sig') as f:
            content = f.read() #读取标注文件内容
            assert '<HRSC_Object>' in content, 'Background picture occurred in %s'%filename #判断是否有无背景
            objects = content.split('<HRSC_Object>')
            info = objects.pop(0)
            for obj in objects:
                assert len(obj) != 0, 'No onject found in %s'%filename
                cls_id = obj[obj.find('<Class_ID>')+10 : obj.find('</Class_ID>')] #获取id
                diffculty = obj[obj.find('<difficult>')+11 : obj.find('</difficult>')]
                if cls_id in ['100000027', '100000022'] or diffculty == '1':
                    continue
                cx = round(eval(obj[obj.find('<mbox_cx>')+9 : obj.find('</mbox_cx>')])) # 获得中心坐标-宽高-角度
                ...
                boxes.append(box)
                label = self.class_mapping(cls_id, self.level) #获得类别
