...
class RetinaNet(nn.Module):
    def __init__(self,backbone='res50',hyps=None):
        ...
        self.num_classes  = int(hyps['num_classes']) + 1 #这里的num-class由分类级别决定,hrsc_dataset.py文件中分三个级别，所以训练时的level也需要改
    ...
    def forward(self, ims, gt_boxes=None, test_conf=None,process=None):
        ...
        original_anchors = self.anchor_generator(ims)   # 尺度=(batchsize, num_all_achors, 5)
        ...
        if self.training: # 如果是训练,则返回损失---分类损失和回归损失---通过loss函数计算
            losses = dict()
            bf_weight = self.calc_mining_param(process, 0.3) # 逐步调整输入iou对匹配度的影响，也就是跟随训练进度来调整alpha
    ...
    def decoder(self, ims, anchors, cls_score, bbox_pred, thresh=0.6, nms_thresh=0.2, test_conf=None):
        if test_conf is not None:
            thresh = test_conf
        bboxes = self.box_coder.decode(anchors, bbox_pred, mode='xywht') #返回真实的预测框，两个坐标以及角度
